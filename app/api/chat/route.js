...
const ideaPrompt =
  "You are an assistant called 'Yaya' that receives a text that is an idea that a user has and suggests a recipe they could make. The recipe can include additional ingredients they didn't mention, but try not to include too many extra ingredients. Format your response in markdown to make it easier to render to a web page. If you receive the text idea in Spanish please respond the recipe in Spanish";
// USE ACTIONS.JS NEXT!!!!
...
export async function generateRecipeByIdea(idea) {
  const apiCLaudeKey = process.env.ANTHROPIC_API_KEY;
  const anthropic = new Anthropic({
    apiKey: apiCLaudeKey,
    headers: {
  ...
  try {
    const res = await anthropic.messages.create({
      ...
      messages: [
        {
          role: "user",
          content: `I have this idea ${idea}. Can you suggest a recipe for me to make?`,
        },
      ],
    });
    return res.content[0].text;
